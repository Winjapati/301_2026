...
-   There are two repos in this class, `301_2026` (the shared course repo) and `LIN_301` (your personal repo)
-   In order to edit each day's slides, you'll need to copy them from 301_2026 into your own

## Setting Up

-   You can do this manually, or run the following command in the terminal (from your `LIN_301` folder), using `bash`:
...
-   Within your `LIN_301/classwork` folder:
    ...
    2.  Move today's slides inside `day4_files`
    3.  Move both `test_folder` and `folder_1` inside `day4_files`
    4.  Download the Terminal command cheat sheet one more time (<https://winjapati.github.io/courses/301/term_comm.txt>), saving it as `cheatsheet.txt`, directly into `day4_files`
    5.  Write and run a script called `check.sh` that lists everything inside `day4_files` when you `source` it