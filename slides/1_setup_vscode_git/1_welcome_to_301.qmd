...
-   A **computer** (PC, Mac, or Linux) with internet access

-   [**VSCode**](https://code.visualstudio.com/download) — our IDE all semester

-   [**Overleaf**](https://www.overleaf.com/) — our LaTeX compiler, no local install needed

-   [**GitHub**](https://github.com/) — version control, and where you'll build your project website
...
A computer might set the unopened jar on top of the unopened bread bag — that step is technically done. What's missing: open the jar, pick up a knife, scoop *how much*, spread it *where*, on *which* slice. That's the level of detail we mean.
...
<!--# 2026-08-24 (post-day-1): finally acted on the 8/19 TODO above (pare down skills list) -- root cause turned out to be timing, not just wordiness: ran out of room for the sandwich activity this year (~6 min left vs. a full block last year). Collapsed the 5 PYTHON SKILLS slides into one, merged VSCode SKILLS + LaTeX SKILLS into one QUARTO & LATEX slide, and folded the 3 "What you'll need" slides into the section-header slide itself -- ~15 slides down to ~6 for this stretch. Also cut spoken-aside commentary that had crept onto the slides (Word/Docs rant, IDE definition, Overleaf endorsement) -- say those live instead. Separately: students this year were unclear on how explicit their sandwich instructions needed to be (didn't come up last year) -- likely because the time crunch meant fewer groups got to see live execution reveal the gaps themselves. Added a concrete "not explicit enough" example (jar-on-unopened-bag) to the first Make Me a Sandwich slide so the standard is set before groups start writing, independent of whether every group gets a live demo. -->