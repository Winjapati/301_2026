...
<!-- Also: make sure to a) show them how to download/install Python *before* class begins & b) that they must select the correct Python interpreter in VSCode (bottom status bar, or Cmd/Ctrl+Shift+P > "Python: Select Interpreter") -->
...
-   You just checked `pip --version`. `pip` is Python's **package installer** — it downloads and installs code that other people have written, so you don't have to write everything yourself.
...
-   You'll do this many times this semester — every time we start using a new tool (`pandas`, `spaCy`, and others), the first step will always be `pip install`.
